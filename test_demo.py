...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        from models.team16_augmented_swinfir import SwinFIR
        model = SwinFIR(
            upscale=4,
            in_chans=3,
            img_size=60,
            ...
            resi_connection='SFB')
        model_path = os.path.join('model_zoo', 'team16_augmented_swinfir.pth')
        ...
        name, data_range = f"{model_id:02}_augmented_swinfir", 1.0
    else:
...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    for i, (img_lr, img_hr) in enumerate(data_path):
        ...
        img_lr = torch.cat([img_lr, torch.flip(img_lr, [2])], 2)[:, :, :h + mod_pad_h, :]
        img_lr = torch.cat([img_lr, torch.flip(img_lr, [3])], 3)[:, :, :, :w + mod_pad_w]
        start.record()
...
def main(args):
    ...
    if True:
        ...
        if args.hybrid_test:
            ...
            #print('inference on the DIV2K and LSDIR test set')
        ...
        mod_pad_h = (h // window_size + 1) * window_size - h
        mod_pad_w = (w // window_size + 1) * window_size - w
        flops = get_model_flops(model, (3, 256 + mod_pad_h, 256 + mod_pad_w), False)
        flops = flops/10**9
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_id", default=16, type=int)
